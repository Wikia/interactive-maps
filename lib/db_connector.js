 ...
 *
 * If you use promise remember about releasing the connection.
 *
...
function getConnection(type, callback, error) {
	...
	poolCluster.getConnection(type, function (err, connection) {
		if (err) {
			...
			// Set the connection as read only if it's not master
			connection.isReadOnly = type !== connType.master;
 ...
 *
 * @param {object} connnection  Database connection
...
function insert(connection, table, data) {
	if (connection.isReadOnly) {
		throw 'Trying to INSERT on READ ONLY connection';
	}
...
function destroy(connection, table, where) {
	if (connection.isReadOnly) {
		throw 'Trying to DELETE on READ ONLY connection';
	}
...
function update(connection, table, data, where) {
	if (connection.isReadOnly) {
		throw 'Trying to UPDATE on READ ONLY connection';
	}
