			...
			.then(function (conn) {
				loadRenderData(req, conn, mapId)
					...
					.then(function (mapData) {
						...
						mapData.isEmbeddedInWikiaApp = typeof(req.query.isEmbeddedInWikiaApp) !== 'undefined';
						// Temporary change required for ad purpose - https://wikia-inc.atlassian.net/browse/DAT-4051
						// We need special handling of protected maps related to the ad campaign.
						// TODO: remove this as a part of https://wikia-inc.atlassian.net/browse/DAT-4055
						if (config.protectedMaps && Array.isArray(config.protectedMaps)) {
							mapData.isProtected = config.protectedMaps.indexOf(mapId) !== -1;
						}
						renderUtils.setupMarkers(mapData.poi_categories);

						res.header({
							'ETag': caching.getEtag(updatedOn.getTime())
						});
						...
						//not passing next on purpose - render will take care of sending error to the user
						crudUtils.releaseConnectionOnFail(conn);
