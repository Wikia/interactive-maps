 ...
 * @returns {object} - promise
 */
function checkTileSetStatus(mapData, language) {
 ...
 */
function handleDefaultCategory(mapData, language) {
	...
	for (i = 0; i < numberOfPoints; i++) {
		if (points[i].poi_category_id === mapData.catchAllCategoryId) {
			...
			types.push({
				...
				marker: null,
				name: i18n.msg('wikia-interactive-maps-default-poi-category', i18n.getLanguage(language)),
			});
			break;
		}
	}
...
function setupMap(req, mapData) {
	...
	handleDefaultCategory(mapData, req.query.uselang);
	...
	mapData.imagesPath = config.getCachebusterUrlPart() + '/vendor/leaflet/images';
...
function middleware(req, res, apiConfigUrl) {
	var mapId = parseInt(req.params.id, 10) || 0,
		language = i18n.getLanguage(req.query.uselang);
	...
	if (mapId !== 0) {
		loadMapInfo(mapId)
			.then(function (mapData) {
				return checkTileSetStatus(mapData, language);
			})
			.then(getPoints)
			.then(getTypes)
			.then(function (mapData) {
