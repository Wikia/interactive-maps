...
function setupMap(req, mapData) {
	...
	if (mapData.type === 'custom') {
		...
		if (mapData.zoom < maxZoom) {
			ratio = Math.pow(2, maxZoom - mapData.zoom);
...
function createAttributionText(mapData) {
	...
	return '<a class="sprite edit" href="' + createEditMapUrl(mapData.city_url, mapData.id) + '" target="_blank">' +
		...
		'<a class="sprite logo" href="http://www.wikia.com" target="_blank">Wikia</a>'+
		'<a href="' + mapData.city_url + '" class="comm">' + communityUrl.hostname + '</a>';
}
