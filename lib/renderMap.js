 ...
 * @returns {number}
 */
function getLatitude(latitude, baundaries) {
	return parseFloat(latitude, 10) || Math.ceil((baundaries.north - baundaries.south) / 2);
}
...
function getLongitude(longitude, baundaries) {
	return parseFloat(longitude, 10) || Math.ceil((baundaries.east - baundaries.west) / 2);
}
			...
			.then(
				function (mapSetup) {
					...
					mapSetup.zoom = getZoomLevel(req.params.zoom, mapSetup.min_zoom,
						utils.binToMaxZoomLevel(mapSetup.max_zoom));
					mapSetup.latitude = getLatitude(req.params.longitude, mapSetup.boundaries);
					mapSetup.longitude = getLongitude(req.params.longitude, mapSetup.boundaries);
					mapSetup.layer = {
						tms: true,
