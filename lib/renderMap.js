 ...
 * @param conn
 * @param mapId
 * @returns {IPromise<U>|Promise<U>|JQueryPromise<U>|Thenable<U>|Promise|JQueryGenericPromise<U>}
 */
function loadRenderData(req, conn, mapId) {
	return loadMapInfo(conn, mapId)
...
function loadRawData(conn, mapId) {
	mapData = {
		id: mapId
	};
	return getPoints(conn, mapData, true)
		.then(function(mapData) {
			return getTypes(conn, mapData, true);
		});
}
...
function renderRawMapData(res, mapData, responseCode) {
	var response = {};
...
function attachEntryPoints(app, apiEntryPointUrlV1, render) {
	...
	app.get(apiEntryPointUrlV1 + 'mapData/:id', mapData);
}
...
function mapData(req, res) {
	...
	if (mapId !== 0) {
		dbCon.getConnection(dbCon.connType.all, function (conn) {
			loadRawData(conn, mapId)
				.then(function (mapData) {
					renderRawMapData(res, mapData, 200);
				});
		});
	}
}
