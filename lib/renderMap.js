...
function checkTileSetStatus(mapData, language) {
	...
	if (mapData.length === 1) {
		...
		if (mapData.status === utils.tileSetStatus.processing) {
			deferred.reject({
				code: 202,
				message: i18n.mesage('wikia-interactive-maps-error-map-processed', language),
		...
		deferred.reject({
			code: 404,
			message: i18n.mesage('wikia-interactive-maps-error-map-not-found', language)
		});
	}
