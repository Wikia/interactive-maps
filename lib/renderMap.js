 ...
 * @returns {object} - promise
 */
function renderMap(res, apiConfigUrl, mapData) {
	var deferred = Q.defer();
 ...
 */
function renderError(res, apiConfigUrl, error) {
...
function middleware(req, res, apiConfigUrl) {
	...
	if (mapId !== 0) {
		loadMapInfo(mapId)
			...
			.then(function(mapData) {
				return renderMap(res, apiConfigUrl, mapData);
			})
			.catch(function (error) {
				renderError(res, apiConfigUrl, error);
			});
	} else {
		res.send(400);
	}
