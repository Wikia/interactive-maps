...
function checkTileSetStatus(mapData, language) {
	...
	if (mapData.length === 1) {
		...
		if (mapData.status === utils.tileSetStatus.processing) {
			deferred.reject({
				code: 202,
				message: i18n.message('wikia-interactive-maps-error-map-processed', language),
		...
		deferred.reject({
			code: 404,
			message: i18n.message('wikia-interactive-maps-error-map-not-found', language)
		});
	}
...
function setupMap(req, mapData) {
	...
	mapData.imagesPath = config.assetsBase || '/vendor/leaflet/images';
