...
module.exports = {
	...
	/**
	 * @desc Generates URL for markers bucket
	 * @param {object} config - map config
	 * @param {number} mapId - map ID
	 * @returns {string} - URL for markers bucket
	 */
	getMarkersPath: function (config, mapId) {
		return 'http://' + config.dfsHost + '/' +
			this.getMarkersBucketName(config.bucketPrefix + config.markersPrefix, mapId) + '/';
	},
