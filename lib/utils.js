...
module.exports = {
	 ...
	 * @returns {Number|String} - URL to the entity
	 */
	...
	/**
	 * @desc Adds a slash to passed string if there is no slash at the end
	 *
	 * @param {string} path
	 * @returns {string}
	 */
	...
	/**
	 * @desc Parses thumbnailer path name to get file name
	 *
	 * @param {string} pathName - Vignette or legacy thumbnailer
	 * @returns {string}
	 */
	getFileNameFromPathName: function (pathName) {
		var vignetteFileNameMatches = pathName.match(/.*\/(.*)\/revision\/.*/);
	 ...
	 * @returns {object}
	 */
	changeMapUpdatedOn: function (conn, dbCon, mapId) {
		return dbCon.update(
			conn,
			'map', {
	 ...
	 * @returns {object}
	 */
	getMapIdByPoiId: function (conn, dbCon, poiId) {
		return dbCon.select(
			conn,
			'poi',
			['map_id'],
	...
	/**
	 * @desc Checks if provided variable is function
	 * @param {*} param Variable to check
	 * @returns {boolean}
	 */
	...
	/**
	 * @desc Returns current unix timestamp
	...
	/**
	 * @desc Sends response with status code and content
	 * @param {Object} res - HTTP response object
	 * @param {Number} code - status code
	 * @param {*} content - response content
	 */
	sendHttpResponse: function (res, code, content) {
		res
			.status(code)
			.send(content)
			.end();
	...
	/**
	 * @desc Decorates each element in passed collection with some additional markers' data
	 * @param {Array} collection collection of POI categories
	 * @param {Object} config configuration of the app
	 * @returns {*}
	 */
	processPoiCategoriesCollection: function (collection, config) {
		this.handleDefaultMarker(collection);
		this.convertMarkersNamesToUrls(
			collection,
			config.dfsHost,
			config.bucketPrefix,
	...
	/**
	 * @desc return process environment
	 * @returns {Object}
	 */
	...
	markJobsAsDelayed: function (jobs) {
		var self = this;
		//mark all active jobs as delayed so kue can pick them up after restart
