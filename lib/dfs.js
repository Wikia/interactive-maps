 ...
 */
function getRandomDFS() {
	return dfsHosts[Math.floor(Math.random() * dfsHosts.length)];
}
 ...
 *
 * @param {String} dfs a DFS machine address ie. s3.dev-dfs-p1 or s3.dev-dfs-p1:80 or 10.14.30.129 or 10.14.30.129:80
 *
 * @return {Array}
 */
function getDFSHostAndPort(dfs) {
	if (dfs === '' || typeof(dfs) === 'undefined') {
		throw new Error('Invalid DFS address.');
	}

	var result = dfs.split(':'),
		port;

	if (result.length > 2) {
		throw new Error('Invalid DFS address. Most probably two many semicolons.');
	}

	port = (!result[1]) ? defaultDfsPort : result[1];
	result[1] = parseInt(port, 10);
...
function createDFSRequestObject(path, method, headers) {
	var dfs = getDFSHostAndPort(getRandomDFS());
...
module.exports = {
	sendFiles: sendFiles,
	getDFSHostAndPort: getDFSHostAndPort
};
