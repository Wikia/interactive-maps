 ...
 *
 * @returns {string} - IP of DFS host
 */
function getRandomDFSHost() {
...
/**
 * @desc Create proper HTTP request object based on fs prox setting
 *
 * @param {string} path
			...
			fs.readFile(dir + '/' + file, function (err, data) {
				...
				var req = http.request(createDFSRequestObject(
					...
					if (res.statusCode === 201) {
						...
						// TODO rejecting doesn't solve the issue, job queue just stops and doesn't retry until restart
						deferred.reject({
							code: res.statusCode
						});
					}
...
function getToken() {
	var deferred = Q.defer(),
		authPath = url.parse(config.swift.config.swiftAuthUrl).path;
