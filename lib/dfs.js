 ...
 */
function getDFSHost(dfsHosts) {
	return dfsHosts[dfsHostIndex++ % dfsHosts.length];
}
...
function getDFSHostAndPort(dfs) {
	...
	var results = url.parse('http://' + dfs, false, false);
...
function createDFSRequestObject(path, method, headers) {
	var dfs = getDFSHostAndPort(getDFSHost(dfsHosts));
