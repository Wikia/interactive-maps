	...
	var dbTable = config.dbTable,
		...
		curd = new CURDCollection({
			...
			fetch: function (req, res, cb) {
				...
				query.then(
					function (collection) {
						...
						object = decorator(collection, config, apiEntryPoint)[0];
			...
			list: function (req, res, cb) {
				...
				query.then(
					function (collection) {
						collection = decorator(collection, config, apiEntryPoint);
...
/**
 * @desc set schemas for object creation and updating
...
function createCustomResponseData(config, type, id) {
	...
	return decorator(object, config, apiEntryPoint)[0];
}
