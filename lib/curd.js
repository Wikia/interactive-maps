...
var dbCon = require( './db_connector' ),
	...
	// TODO: temporary should be moved to other place
...
function createCollection( config ) {
	...
	curd = new CURDCollection({
		...
		fetch: function( req, res, cb ) {
			...
			query.then(
				function( data ) {
					// add api urls to data objects
					var obj = decorator( data, responseSchema, apiEntryPoint );

					// if no rows found, send 404
					( obj[0] ) ? cb( null, obj[0] ) : cb( true );
				},
				function( err ) {
					res.status.internalServerError( err );
				}
		...
		list: function( req, res, cb ) {
			...
			query.then(
				function( data ) {
					// add api urls to data objects and send response to client
					cb( null, decorator( data, responseSchema, apiEntryPoint ) );
				},
				function( err ) {
					res.status.internalServerError( err );
				}
