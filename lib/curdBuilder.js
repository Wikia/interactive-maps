...
function createResponseData(resData, id) {
	...
	if (resData.hasOwnProperty('id') && id) {
		resData.id = id;
	}
		...
		curd = {
			handler: {
				GET: function (req, res, next) {
					...
					query.then(
						function (collection) {
							collection = responseDataDecorator(
								collection,
								responseSchema,
								createEntryPointUrl(req, curdUrlPath),
								transforms
							);
					...
					if (errors.length === 0) {
						...
						query.then(
							function (data) {
								...
								data = responseDataDecorator(
									createResponseData(customResObjects.create, data),
									responseSchema,
									createEntryPointUrl(req, curdUrlPath)
								)[0];
			...
			wildcard: {
				...
				GET: function (req, res, next) {
					...
					if (isFinite(id)) {
						query.then(
							function (collection) {
								var object = responseDataDecorator(
									collection,
									responseSchema,
									createEntryPointUrl(req, curdUrlPath),
									transforms
