...
function addToDB(dbTable, data) {
	...
	return dbCon.insert(dbTable, tileSetObject);
}
 ...
 * @returns {object} promise
 */
function checkIfTileSetExists(dbTable, data) {
...
function setupProcessJob(dbTable, data, id) {
	...
	var job = jobs.create('process', {
		...
		.on('complete', function () {
			...
			updateTileSetStatus(dbTable, job.data.tileSetId, utils.tileSetStatus.failed);
 ...
 * @returns {object} promise
 */
function insertSearchData(dbTable, tileSetId, tileSetName) {
	return dbCon.insert(
		dbTable + '_search',
		{
			id: tileSetId,
...
module.exports = function process(dbTable, data) {
	...
	checkIfTileSetExists(dbTable, data).then(function (result) {
		if (result.length) {
			...
			addToDB(dbTable, data)
				.then(function (result) {
					...
					setupProcessJob(dbTable, data, tileSetId);
					insertSearchData(dbTable, tileSetId, data.name);
