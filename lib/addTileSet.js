 ...
 * @param {integer} status to set for the map
 */
function updateFailedMapStatus(dbTable, id, status) {
	dbCon.update(dbTable, {
		id: id
	}, {
		status: status
	}).then(function () {
		logger.error('Updated tile set status with id: ' + id + ', to ' + status);
	});
}
...
function setupProcessJob(dbTable, data, id) {
	...
	var job = jobs.create('process', {
		...
		.on('complete', function () {
			...
			updateFailedMap(dbTable, job.data.mapId, failedStatus);
