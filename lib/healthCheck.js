 ...
 */
function getDbNodeHeartbeat(connType) {
	...
	dbCon.getConnection(connType)
		...
		.catch (function () {
			deferred.resolve(connType === dbCon.connType.master ? mysqlMasterFailed : mysqlSlaveFailed);
		});
...
function getDbHeartbeat(res) {
	Q.allSettled([
		...
		getDbNodeHeartbeat(dbCon.connType.slave)
