...
/**
 * @desc Exit process when the max number of tiling jobs is reached. It will be respawned by parent automatically.
 * This is to keep service running even if there are memory leaks.
 */
function restartOnMaxNumberOfJobs() {
	tilingJobsCounter++;

	if (tilingJobsCounter >= maxJobsPerChild) {
		jobs.shutdown(function () {
			logger.debug('Max number of tiling jobs per child was reached. Restarting the child process.');
			logger.close();
			process.exit(0);
		}, 1000);
	}
