...
/**
 * @desc Updates the tile set's background color in the database
 * @param {string} dbTable
 * @param {string} id of a tile set
 * @param {string} color to set for the tile set
 */
function updateTileSetBgColor(dbTable, id, color) {
	dbCon.getConnection(dbCon.connType.master, function (conn) {
		dbCon.update(conn, dbTable, {
...
function createJob(data, priority, attempts, delay) {
	jobs.create(machineJobName('tiling'), data)
		...
		.on('complete', function () {
			...
			updateTileSetBgColor(dbTable, data.tileSetId, data.bgColor);
