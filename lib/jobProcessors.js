...
function restartOnMaxNumberOfJobs() {
	...
	logger.error('Jobs counter for worker ' + process.pid + ': ' + jobsCounter);
	if (jobsCounter >= maxJobsPerChild) {
		jobs.shutdown(function () {
			logger.debug('Max number of jobs per child was reached. Restarting the child process.');
			logger.close();
			process.exit(0);
		}, 1000);
	}
