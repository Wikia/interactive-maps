...
var dbCon = require('./db_connector'),
	...
	celeryUpdate = require('./celeryUpdate'),
...
function createJob(data, priority, attempts, delay) {
	jobs.create(machineJobName('tiling'), data)
		...
		.on('complete', function () {
			...
			celeryUpdate.purgeKey(utils.surrogateKeyPrefix + data.mapId, 'tilesSetCreated');
		}).on('failed', function () {
...
jobs.process('process_marker', config.kue.maxProcessMarkerJobs, function (job, done) {
	...
	fetchImage(job.data)
		...
		.then(function () {
			...
			celeryUpdate.purgeKey(utils.surrogateKeyPrefix + job.data.mapId, 'markerUpdated');
		})
		.catch(function (err) {
			context = {
				job: 'process_marker',
