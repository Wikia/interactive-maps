...
taskBrokerConf = env.WIKIA_CONFIG_ROOT + '/TaskBroker.yml';
...
try {
	...
	taskBrokerConfiguration = yaml.safeLoad(
		fs.readFileSync(taskBrokerConf, charset)
	);
...
configuration.swift = swiftConfiguration.wgFSSwiftDC[yamlEnv][env.WIKIA_PROD_DATACENTER.toLowerCase()];
configuration.taskBroker = taskBrokerConfiguration.wgTaskBroker[yamlEnv];
