...
jobs.process( 'process', config.kue.maxFetchJobs, function( job, done ) {
	fetchImage( job.data )
		.then(function( data ) {
			// pass done callback to tiling job so it's called when tiling if finished
			setupTiling( data, done );
		});
});
 ...
 * @param processJobDone {function} - 'done' callback for process job
 * @returns {object} - promise
 */

function setupTiling( data, processJobDone ) {
	...
	job.on( 'complete', function() {
		// triggering 'complete' event for 'process' job
		processJobDone();
	}).on( 'failed', function() {
		// triggering 'failed' event for 'process' job
		processJobDone(true);
	});
...
function insertMap( data ) {
	...
	if ( data.firstJob ) {
		...
		query = dbCon.ins( dbTable, object );
	...
	// update map in DB with higher zoom levels
		...
		query = dbCon.upd( dbTable, object, where );
...
module.exports = function process( data ) {
	...
	job.on( 'complete', function() {
		...
		// TODO: figure out what should be sent to the client
		deferred.reject();
	});
